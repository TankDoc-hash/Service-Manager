...
export async function POST(req: NextRequest) {
  try {
    ...
    response.cookies.set('tankdoc_token', token, {
      httpOnly: true,
    ...
    console.error('[POST /api/auth/login]', message, error)
    return NextResponse.json({ error: 'Login failed', debug: message }, { status: 500 })
  }
}
